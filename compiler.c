...
static void emitLoop(int loopCount) {
  emitByte(OP_LOOP);

  //  the +2 is to account for the operands of the OP_LOOP instruction
  int offset = currentChunk()->count - loopCount + 2;
...
static uint8_t argumentList() {
  ...
  consume(TOKEN_RIGHT_PAREN, "Expect ')' after the function parameter list");
  return argCount;
}
...
static void addLocal(Token* name) {
  if (current->localCount > UINT8_COUNT) {
    error("Too many local variables defined in function");
    return;
  }
...
static int addUpvalue(Compiler* compiler, uint8_t index, bool isLocal) {
  ...
  if (upvalueCount == UINT8_COUNT) {
    error("Too many closure variables defined in this function");
    return 0;
  }
...
static int resolveUpvalue(Compiler* compiler, Token* name) {
  ...
  int index = resolveLocal((Compiler *)compiler->enclosing, name);
  if (index != -1) {
...
static void declareVariable() {
  //  Don't declare global variables here
  if (current->scopeDepth == 0) return;

  Token* name = &parser.previous;

  //  check if a variable is being re-declared in the same local scope
  for(int i = current->localCount - 1; i >=0; i--) {
    Local* local = &current->locals[i];

    //  break out of the loop if the variables are in an outer scope to the current one
    ...
    if (identifiersEqual(name, &local->name)) {
      error("Trying to redeclare an already defined variable in the same scope");
    }
  }
...
static void printStatement() {
  expression();
  consume(TOKEN_SEMICOLON, "Expect semicolon at the end of the print statement");
  emitByte(OP_PRINT);
}
...
static void whileStatement() {
  ...
  consume(TOKEN_LEFT_PAREN, "Expect '(' before while expression");
  expression();
...
static void forStatement() {
  beginScope();
  consume(TOKEN_LEFT_PAREN, "Expect '(' before for clause");
  ...
  if (!check(TOKEN_RIGHT_PAREN)) {
    do {
      ...
      if (current->function->arity > 255) {
        error("Too many arguments in this function");
      }
...
static void funDeclaration() {
  uint8_t global = parseVariable("Function name expected");
  markInitialized();
